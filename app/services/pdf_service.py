...
MAX_PAGES = 2000
WIDTHS = (400, 600, 800, 1000, 1200, 1400, 1600, 2000)   # 요청 폭은 이 단계로 맞춰 캐시 적중률을 높인다
RENDER_TIMEOUT = 30
...
async def _run(*args: str) -> bytes:
    process = await asyncio.create_subprocess_exec(
        *args, stdout=asyncio.subprocess.PIPE, stderr=asyncio.subprocess.PIPE, stdin=asyncio.subprocess.DEVNULL
    )
    try:
    ...
    if process.returncode != 0:
        ...
        raise FsError("PDF 를 읽지 못했습니다." + (f" ({message[-1][:120]})" if message else ""))
    return out
...
async def info(path: str) -> dict:
    ...
    sizes = []
    if pages > 1:
        detail = (await _run("pdfinfo", "-f", "1", "-l", str(pages), str(file))).decode(errors="replace")
        sizes = [(float(w), float(h)) for w, h in re.findall(r"^Page\s+\d+\s+size:\s+([\d.]+) x ([\d.]+)", detail, re.M)]
    if not sizes:
        match = re.search(r"^Page size:\s+([\d.]+) x ([\d.]+)", head, re.M)
        sizes = [(float(match.group(1)), float(match.group(2)))] if match else [(612.0, 792.0)]
...
def _prune() -> None:
    root = cache_dir()
    try:
        files = sorted((p for p in root.rglob("*.png")), key=lambda p: p.stat().st_atime)
...
async def page(path: str, number: int, width: int) -> Path:
    ...
    async with _render_slots:
        ...
        png = await _run(
            "pdftoppm", "-png", "-f", str(number), "-l", str(number), "-singlefile",
            "-scale-to-x", str(width), "-scale-to-y", "-1", "-aa", "yes", "-aaVector", "yes", "-hide-annotations", str(file),
        )
        if not png:
