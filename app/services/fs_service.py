...
SEARCH_LIMIT = 200
SEARCH_SECONDS = 3.0
SEARCH_SKIP = {".git", "node_modules", ".venv", "venv", "__pycache__", ".cache", ".mypy_cache", ".pytest_cache", ".ruff_cache", ".tox", "dist", "build", ".next"}
IMAGE_EXT = {".png", ".jpg", ".jpeg", ".gif", ".webp", ".bmp", ".ico", ".svg", ".avif"}
MEDIA_EXT = {".mp4", ".webm", ".mov", ".m4v", ".mp3", ".wav", ".m4a", ".ogg", ".oga", ".flac", ".aac"}
...
def roots() -> list[Path]:
    configured = [p for p in os.getenv("EXPLORER_ROOTS", "").split(os.pathsep) if p.strip()]
    paths = [Path(os.path.expanduser(p)).resolve() for p in configured] or [Path.home().resolve()]
...
def resolve(path: str, must_exist: bool = True) -> Path:
    ...
    real = candidate.resolve() if os.path.exists(candidate) else candidate.parent.resolve() / candidate.name
...
def _git_ignored(directory: Path, names: list[str]) -> set[str]:
    ...
    try:
        result = subprocess.run(
            ...
            input="\0".join(rel).encode() + b"\0", capture_output=True, timeout=3,
...
def list_dir(path: str) -> dict:
    ...
    return {
        "path": str(directory),
        "parent": str(parent) if parent != directory and _inside(parent.resolve()) else None,
        "entries": entries,
...
def read_file(path: str) -> dict:
    ...
    info = _entry(file) or {}
    info["mtime_ns"] = str(file.stat().st_mtime_ns)  # JS 숫자(2^53)로는 정밀도가 모자라 문자열로
    ext = file.suffix.lower()
    info["mime"] = mimetypes.guess_type(file.name)[0] or "application/octet-stream"
    info["image"] = ext in IMAGE_EXT
    info["pdf"] = ext == ".pdf"
    info["media"] = ("video" if ext in {".mp4", ".webm", ".mov", ".m4v"} else "audio") if ext in MEDIA_EXT else None
...
def delete(path: str) -> str:
    ...
    _protected(source)
    trash = trash_dir()
    if trash.resolve() in source.resolve().parents or source.resolve() == trash.resolve():
...
def transfer(path: str, dest_dir: str, move: bool) -> str:
    ...
    if source.is_dir() and (directory.resolve() == source.resolve() or source.resolve() in directory.resolve().parents):
...
def write_file(path: str, content: str, expected_mtime_ns: int | None) -> dict:
    ...
    temp = real.with_name(f".{real.name}.univdash-{os.getpid()}-{time.monotonic_ns()}.tmp")
    try:
        fd = os.open(temp, os.O_WRONLY | os.O_CREAT | os.O_EXCL, statmod.S_IMODE(st.st_mode))
...
def _git_files(directory: Path) -> list[Path] | None:
    ...
    try:
        result = subprocess.run(
            ["git", "-C", str(directory), "ls-files", "-z", "--cached", "--others", "--exclude-standard"],
    ...
    if result.returncode != 0:
        return None
    return [directory / name for name in result.stdout.decode(errors="replace").split("\0") if name]
...
def zip_folder(path: str, skip_ignored: bool = False) -> tuple[Path, str]:
    ...
    for file in files:
        ...
        if total > ZIP_MAX_BYTES:
            raise FsError("폴더가 너무 큽니다 (최대 4GB). .gitignore 제외 옵션이나 하위 폴더로 받아 보세요.")
        chosen.append(file)
    ...
    try:
        with zipfile.ZipFile(temp, "w", compression=zipfile.ZIP_DEFLATED, compresslevel=6, allowZip64=True) as archive:
            for file in chosen:
                try:
                    archive.write(file, f"{base}/{file.relative_to(directory).as_posix()}")
...
CLONE_URL = re.compile(r"^(https?://[^\s]+|ssh://[^\s]+|git://[^\s]+|[\w.\-]+@[\w.\-]+:[^\s]+)$")
CLONE_TIMEOUT = 900
...
def git_clone(url: str, parent: str, name: str = "", branch: str = "", depth: int = 0) -> dict:
    """parent 폴더 안에 git clone. 비밀번호 창은 띄우지 않는다(ssh 키 · 저장된 자격 증명만 사용)."""
    url = (url or "").strip()
    if not CLONE_URL.match(url) or url.startswith("-") or len(url) > 2000:
        raise FsError("저장소 주소가 올바르지 않습니다. (https://… 또는 git@host:user/repo.git)")
    directory = resolve(parent)
    if not directory.is_dir():
        raise FsError("대상이 폴더가 아닙니다.")
    target = directory / valid_name(name or clone_name(url))
    if os.path.lexists(target):
        raise FsError(f"'{target.name}' 이(가) 이미 있습니다. 다른 폴더 이름을 적어 주세요.")
    branch = (branch or "").strip()
    if branch and (branch.startswith("-") or not re.fullmatch(r"[\w./\-]{1,200}", branch)):
    ...
    try:
        result = subprocess.run(args, cwd=directory, capture_output=True, text=True, timeout=CLONE_TIMEOUT, env=env, stdin=subprocess.DEVNULL)
    except subprocess.TimeoutExpired as error:
        shutil.rmtree(target, ignore_errors=True)
        raise FsError("clone 시간이 너무 깁니다 (15분 초과).") from error
    if result.returncode != 0:
        shutil.rmtree(target, ignore_errors=True)
        lines = [line for line in result.stderr.splitlines() if line.strip() and not line.startswith(("Cloning into", "remote:", "Receiving", "Resolving"))]
        detail = lines[-1] if lines else "알 수 없는 오류"
        if "Authentication failed" in result.stderr or "could not read Username" in result.stderr or "Permission denied" in result.stderr:
...
def properties(path: str) -> dict:
    ...
    if is_dir:
        try:
            ...
            info["child_dirs"] = sum(1 for c in children if c.is_dir(follow_symlinks=False))
        except OSError:
            info["children"] = None
    repo = _repo_of(target.resolve() if is_dir else target.resolve().parent)
    if repo is not None:
        info["git_repo"] = str(repo)
        info["git_ignored"] = bool(_git_ignored(target.parent, [target.name])) if target != repo else False
    return info
...
async def folder_usage(path: str) -> dict:
    ...
    async def du(*flags: str) -> tuple[int | None, bool]:
        ...
        try:
            ...
            return None, True
        return value, process.returncode != 0 or bool(err.strip())   # 권한 없는 하위 폴더가 있으면 일부만 센 값

    if not shutil.which("du"):
        raise FsError("서버에 du 가 없습니다.")
    (size, p1), (disk, p2), (inodes, p3) = await asyncio.gather(du("-b"), du("-B1"), du("--inodes"))
    result = {
        "size": size, "disk": disk,
        "items": max(0, inodes - 1) if inodes is not None else None,   # 자기 자신 제외
        "partial": p1 or p2 or p3, "timed_out": size is None,
